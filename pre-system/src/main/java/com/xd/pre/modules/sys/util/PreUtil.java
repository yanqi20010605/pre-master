...
public class PreUtil {
     ...
     * @param classes
     */
    public void findChildren2(List<SysClass> sysClasses, List<SysClass> classes) {

        for (SysClass sysClass : sysClasses) {
            List<SysClass> children = new ArrayList<>();
            for (SysClass class1 : classes) {
                if (sysClass.getClassId() != null && sysClass.getClassId().equals(class1.getParentId())) {
                    class1.setParentName(sysClass.getClassName());
                    class1.setLevel(sysClass.getLevel() + 1);
                    children.add(class1);
                }
            }
            sysClass.setChildren(children);
            findChildren2(children, classes);
        }
    }
     ...
     * @param classes
     */
    public void findChildren3(List<ClassTreeVo> sysClasses, List<SysClass> classes) {

        for (ClassTreeVo sysClass : sysClasses) {
            sysClass.setId(sysClass.getId());
            sysClass.setLabel(sysClass.getLabel());
            List<ClassTreeVo> children = new ArrayList<>();
            for (SysClass class1 : classes) {
                if (sysClass.getId() == class1.getParentId()) {
                    ClassTreeVo classTreeVo1 = new ClassTreeVo();
                    classTreeVo1.setLabel(class1.getClassName());
                    classTreeVo1.setId(class1.getClassId());
                    children.add(classTreeVo1);
                }
            }
            sysClass.setChildren(children);
            findChildren3(children, classes);
        }
    }
